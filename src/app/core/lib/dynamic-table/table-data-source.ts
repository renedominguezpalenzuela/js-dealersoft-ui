...
export class MatTableDataSource<T> extends DataSource<T> {
  ...
  sortingDataAccessor: ((data: T, sortHeaderId: string) => string | number) = (data: T, sortHeaderId: string): string | number => {
    const value = new NestedPropertyPipe().transform(data, sortHeaderId);
  ...
  _updateChangeSubscription() {
    ...
    const sortChange: Observable<Sort | null | void> = this._sort ?
      merge(this._sort.sortChange, this._sort.initialized) : observableOf(null);
    const pageChange: Observable<PageEvent | null | void> = this._paginator ?
      merge(this._paginator.page, this._paginator.initialized) : observableOf(null);
