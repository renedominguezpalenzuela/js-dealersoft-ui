...
import { ApiHelperService, LoadingService, NotificationService, RequestService } from '@core/services';
...
export class CustomerInformationComponent implements OnInit {
  ...
  public loadPaginatedData = ($event: number) => {
    ...
    this.requestService.Get(this.apiHelperService.clientsURL, this.query())
      .subscribe(res => {
        this.data = res.data;
  ...
  private editCustomer(customer: Customer) {
    const subscription = () => {
      this.notificationService.riseNotification({ color: 'success', data: 'Kunde erfolgreich aktualisiert' });
      this.loadPaginatedData(this.currentPage);
    };
    this.matDialog.open(CustomerFormComponent, {
      width: '650px',
      ...
      if (typeof out !== 'boolean' && typeof out !== 'undefined') {
        ...
        this.requestService.Put(`${ this.apiHelperService.clientsURL }/${ customer.id }`, value)
  ...
  private removeCustomer(customer: Customer) {
    this.matDialog.open(RemoveConfirmationComponent, {
      ...
      if (out) {
        this.requestService.Delete(`${ this.apiHelperService.clientsURL }/${ customer.id }`)
          .subscribe(() => {
            this.loadPaginatedData(this.currentPage);
            this.notificationService.riseNotification({ color: 'success', data: 'Kunde erfolgreich entfernt' });
          });
      }
    });
  }
