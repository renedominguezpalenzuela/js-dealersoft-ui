...
export class AdminComponent implements OnInit {
  ...
  constructor(
    ...
    this.router.events
      ...
      .subscribe((event) => this.titlePage = this.links.find(link => event.urlAfterRedirects.includes(link.url))?.label ?? 'Administrator');
    this.authService.currentUser.subscribe((user) => {
      this.isAuth = this.authService.isAuth;
      this.authUser = user;
      this.days = moment(this.authUser?.active_until).diff(moment(), 'days');
      if (this.authUser && this.isAuth)
        this.requestService.Get(this.apiHelperService.logosURL, this.queryLogo(<number>this.authUser?.id))
          .subscribe(logos => {
  ...
  public logOut() {
    ...
    this.notificationService.riseNotification({ color: 'success', data: 'Sitzung erfolgreich geschlossen' });
  ...
  public toggleSide() {
    ...
    if (this.showSide) {
      this.renderer2.removeClass(this.drawerSide.nativeElement, 'animate__fadeInLeft');
      this.renderer2.addClass(this.drawerSide.nativeElement, 'animate__fadeOutLeft');
      setTimeout(() => {
        this.renderer2.setStyle(this.drawerSide.nativeElement, 'display', 'none');
      }, 250);
    } else {
      this.renderer2.removeClass(this.drawerSide.nativeElement, 'animate__fadeOutLeft');
      this.renderer2.addClass(this.drawerSide.nativeElement, 'animate__fadeInLeft');
      this.renderer2.setStyle(this.drawerSide.nativeElement, 'display', 'initial');
    }
  }
